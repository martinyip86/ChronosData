...
class DailyPatcher:
    def __init__(self,target_date:str):
        ...
        self.logger = setup_logger('daily.patcher')
    ...
    def check_data_exists(self,target_date_str:str,symbol):
        sql = f"""
            SELECT 
                trade_id 
            FROM 
        ...
        if not result.result_rows:
            self.logger.warning(f"🚨 {symbol} 在 {target_date_str} 数据严重缺失，启动全量同步")
            file_path = self.download_and_unzip('binance',symbol,target_date_str)
            if file_path and os.path.exists(file_path):
                try:
                    ...
                    self.sync_to_clickhouse(symbol,official_df)
                    self.logger.info(f"✅ {symbol} 补丁同步完成: 缝合 {len(official_df)} 条数据")

                    del official_df
                    return True
                except Exception as e:
                    self.logger.error(f"❌ 处理 {symbol} 补丁时出错: {e}")
                finally:
    ...
    def main(self):
        ...
        for symbol in self.symbols:
            flag = self.check_data_exists(self.date_str,symbol)
            gc.collect()
            if flag: continue

            gaps = self.get_ch_data(self.date_str,symbol)
            
            file_path = self.download_and_unzip('binance',symbol,self.date_str)
            if file_path and os.path.exists(file_path):
                official_df = pl.scan_csv(file_path,has_header=False,new_columns=["trade_id","price","amount","cost","timestamp","is_maker","is_best"]).collect()
                validator_trades(official_df)
                if gaps:
                    try:
                        ...
                        if all_patch_data:
                            ...
                            self.sync_to_clickhouse(symbol,final_df)
                            self.logger.info(f"✅ {symbol} 补丁同步完成: 缝合 {len(final_df)} 条数据")

                    except Exception as e:
                        self.logger.error(f"❌ 处理 {symbol} 补丁时出错: {e}")
                        

                self.verify_full_integrity(symbol=symbol,official_df=official_df,file_path=file_path)
    ...
    def get_ch_data(self,target_date_str:str,symbol):
        ...
        if result.result_rows:
            for row in result.result_rows:
                self.logger.warning(f"🚨 发现空洞: {symbol} | 范围: {row[1]} <-> {row[2]} | 缺失: {row[0]}条")
            return result.result_rows
        else:
            self.logger.info(f"✅ {symbol} 数据在 ClickHouse 中完美连贯。")
            return []
    ...
    def sync_to_clickhouse(self,symbol,df:pl.DataFrame):
        ...
        self.logger.info(f"✅[binance-{symbol}] 准备写入 {len(formatted_df)} 条数据!")
        try:
            ...
            self.logger.info(f"✅[binance-{symbol}] 成功缝合 {len(formatted_df)} 条数据!")
    ...
    def verify_full_integrity(self,symbol,official_df:pl.DataFrame,file_path):
        try:
            self.logger.info(f"🔍 开始全属性核对: {symbol} @ {self.date_str}")
            ...
            sql = f"""
                ...
                FROM
                    trades FINAL
                WHERE exchange_id='binance' AND symbol='{symbol}'
            ...
            if ch_df.is_empty():
                self.logger.error(f"❌ 核对失败：数据库中未找到 {symbol} 的任何数据")
                return False
            ...
            if diff.is_empty() and len_csv_df == len_ch_df:
                self.logger.info(f"✅ [全属性对账通过] {symbol} 共 {len_ch_df} 条数据，每一位都与官方一致。")
                if os.path.exists(file_path):
                    os.remove(file_path)
                return True
            else:
                self.logger.error(f"🚨 [对账失败] {symbol} 发现 {len(diff)} 条差异或缺失数据！")
                # 保存差异样本供排查
                diff_sample_path = f"temp/diff_{symbol}_{self.date_str}.csv"
